...
Below we fix a well-formed PTA $\pta$ in the form (\ref{eq:pta}) and a TRA (or resp. TFA) $\dta$ in the form (\ref{eq:tra}) (or resp. (\ref{eq:tfa})).
...
\begin{align*}
&\exttuples_{\dtloc}:=\{h:\alphabet\rightarrow\clcons{\clocks_2}\mid\\
&\qquad
\forall b\in\alphabet.\left(\dtloc, b, h(b), X, \dtloc')\in\rules\mbox{ for some }X, \dtloc'\right)\}\enskip.
\end{align*}
The totality condition ensures that $\exttuples_{\dtloc}$ is non-empty. 
...
& \mbox{is a rule in }\rules\\%\mbox{ and }R'={R}{\left[Y\cap \clocks_2:=0\right]}  \\
0 & \mbox{otherwise}
\end{cases}\enskip.
\end{align*}
