...
Below we prove the hardness of the problem. In \cite{LaroussinieS07}, ?? proved that the reachablity-probability problem for arbitrary PTAs is \emph{EXPTIME}-complete. Reduction from the PTA reachibility problem to the PTA-DTA problem can be easily constructed as follows.
...
We also construct the DTA $\dta'$ by
\[
\dta':=\left(\{q_0,q_1\},2^{AP\cup\{\mbox{acc\}}},\emptyset,\rules,\{(\emptyset, \{q_1\}\}\right)
\]
where $\rules$ contains exactly the following rules:
\begin{compactitem}
    \item 
