...
\begin{remark}[Technical Novelty]
...
\end{proof}
A side result from Theorem~\ref{thm:main} says that $\sfunc$ preserves time-divergence for schedulers before and after product construction.
From Theorem~\ref{thm:main} and Lemma~\ref{lemm:sfuncbij}, one immediately obtains the following result which transforms the {\sc PTA-DTA} problem into Rabin(-accepting) probabilities under the product PTA.
...
Below we prove the hardness of the problem. In \cite{LaroussinieS07}, ?? proved that the reachablity-probability problem for arbitrary PTAs is \emph{EXPTIME}-complete.
...
It follows that the maximum/minimum probability to reach $\fstates$ can be solved by the algorithm demonstrated in Proposition~\ref{?} for Rabin acceptance condition.
