...
Finally, we label each $(\loc,\dtloc)$ by $\dtloc$ to meet the Rabin or finite acceptance condition.\qed
...
\begin{example}
The product PTA between the PTA in Example~\ref{ex:pta} and the DTA in Example~\ref{ex:dta} is depicted in Figure~\ref{fig:product}.
In the figure, $\PairS{\WORK{\alpha}}{\q{\alpha}}, \PairS{\WORK{\beta}}{\q{\beta}}$ and 
$\PairS{\WORK{\alpha}}{\FAIL}, \PairS{\WORK{\beta}}{\FAIL}$ 
are product locations. 
...
\item $h_4=\{\{ \beta \} \mapsto y \le C_\beta ,\{ \alpha  \} \mapsto W_\alpha< y\}$;
\item $h_5=\{\{ \beta \} \mapsto y \le C_\beta ,\{ \alpha  \} \mapsto W_\beta< y\}$;
...
Below we prove the hardness of the problem. It is proved in \cite{LaroussinieS07} that the reachablity-probability problem for arbitrary PTAs is \emph{EXPTIME}-complete.
We show a polynomial-time reduction from the PTA reachibility problem to the PTA-DTA problem as follows.
...
we let $\pta'=\left(\locs, \loc^*, \clocks, \acts, \inv, \enab,  \prob, \ap',\lbfunc'\right)$ where $\ap':=\ap\cup\{\mbox{\sl acc}\}$ and
...
\begin{compactitem}
    \item  $(q_0,U,true, \emptyset,q_1)\in\rules$ for all $U\in\alphabet$ such that $\mbox{acc}\in U$;
%$\forall \sigma\in\Sigma, \mbox{acc}\in \sigma \rightarrow (q_0,\sigma,true, \emptyset,q_1)\in\rules$,
    \item $(q_i,U,true, \emptyset,q_i)\in\rules$ for all $U\in\alphabet$ and $i\in\{0,1\}$.
\end{compactitem}
It is then straightforward from definition that an infinite path under $\pta$ visits some location in $\fstates$ iff the infinite path (under $\pta'$) is accepted by $\dta'$.
...
It follows that the maximum/minimum probability to reach $\fstates$ can be solved by the algorithm demonstrated in Proposition~\ref{prop:exptime} for Rabin acceptance condition.
By the reduction, we obtain the main result of this section which settles the computational complexity of the problem PTA-DTA.


\begin{theorem}
The PTA-DTA problem is EXPTIME-complete.
\end{theorem}
%
%\vspace{-0.8em}
%
